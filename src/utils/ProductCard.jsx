               ...
               <div className="relative aspect-[6/6] overflow-hidden rounded-sm transition-all duration-200 ease-in-out">
                    ...
                    {product.discount && (
                         <div className="absolute top-14 left-3">
                              <span className="bg-primary text-accent text-[11px] rounded-sm font-bold px-3 py-1.5">
                                   {product.discount}
                              </span>
                         </div>
                    )}
                    {
                         product.badge && (
                              < span className="absolute top-4 left-3 bg-primary/90 backdrop-blur-xl text-accent text-[11px] font-bold px-3 py-1.5 uppercase tracking-wider rounded-sm">
                                   {product.badge}
                              </span>
                         )
                    }
                    {/* Wishlist Icon */}
                    <button className={`absolute top-4 right-3 p-2.5 bg-secondary/85 md:opacity-0 group-hover:opacity-100 backdrop-blur-lg rounded-full text-accent transition-all duration-200 hover:bg-primary/90 hover:text-accent active:bg-primary/90 active:text-accent hover:scale-115 active:scale-115`}>
                         <FiHeart size={16} />
                    </button>
                    {/* Quick Add Button */}
                    <div className="absolute bottom-0 left-0 w-full p-2.5 sm:p-4 translate-y-0 sm:translate-y-full group-hover:translate-y-0 transition-transform duration-600 ease-out">
